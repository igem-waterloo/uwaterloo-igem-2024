numsimulations = 1;
AA = [zeros([numsimulations 133])];
BB = [zeros([numsimulations 137])];
CC = [zeros([numsimulations 141])];
DD = [zeros([numsimulations 129])];
figure
hold on
...
for i = 1:n
    randomNumbers(i) = ranges(i, 1) + (ranges(i, 2) - ranges(i, 1)) * rand();
end
...
ode_system = @(t, y) [
  ...
  %y(2) corresponds to ST
  %dST_dt = INST_interp(t) - STGLU - OUTST
  %INST_interp(t) - KST * y(2) - KOUTST * y(2);

  %y(3) corresponds to GLU
  %dGLU_dt = CWGLU + STGLU + PYGL - GLPY - OUTGLU
  %(KCW * y(1)) + (KST * y(2)) + ((2E10/Keq1) * (PYR)^2 * (NADH)^2 * (ATP)^2 * exp(-C * DEGLU)) - (2E10 * y(3) * (NAD)^2 * (ADP)^2 * exp(C * DEGLU)) - (0.0001 * y(3));

  %y(4) corresponds to PYR
  %dPYR_dt = 2*GLPY - 2*PYGL + ACPY - PYAC + LAPY - PYLA + 2*BUPY - 2*PYBU + PROPY - PYPRO + 2*VALPY - 2*PYVAL - PYMIC - OUTPY
  %2*GLPY - 2*PYGL + ACPY - (10000 * k * y(4) * NAD * ADP * exp(C * DEACE)) + LAPY - (k * y(4) * NADH * exp(C * DEPYR)) + 2*BUPY - 2*(1000 * k * (y(4))^2 * ADP) + PROPY - (100 * k * y(4) * ADP * (NADH)^2 * exp(C * DEPRO)) + 2*VALPY - 2*(k * (y(4))^2 * (NADH)^3 * exp(C * DEVAL)) - PYMIC - (KA2 * y(4));

  %y(5) corresponds to LAC
  %dLAC_dt = -LAPY + PYLA - LAPRO + PROLA - LACA + ALAC - OUTLAC
  %-(LAPY) + PYLA - (k * y(5) * NADH * exp(C * DELAC)) + PROLA - (KA * y(5) * HPLUS) + ALAC - (KA2 * y(5));


];

%Initial conditions: [CW, ST, GLU, PYR
%initial_conditions = [CW, ST, GLU, PYR, LAC];
initial_conditions = [CW];
%Solve the system of DEs using ode45
[t_sol, y_sol] = ode45(ode_system, [0 t_end], initial_conditions);

%soln = ode45(ode_system, [0 t_end], initial_conditions)
...
%Check the values of the solved ODE
%disp('LAC Values')
%disp(LAC_sol)
%Plot the solution
%figure;
%plot(t_sol, CW_sol, 'b', 'DisplayName', 'CW');

plot(t_sol, CW_sol/y_sol(21), 'r', 'DisplayName', 'CW_percentage');
for j = 1:size(y_sol,1)
    if(size(y_sol,1)==size(AA,2))
        AA(i,j)=y_sol(j);
    elseif(size(y_sol,1)==size(BB,2))
        BB(i,j)=y_sol(j);
    elseif(size(y_sol,1)==size(CC,2))
       CC(i,j)=y_sol(j);
    else
        DD(i,j)=y_sol(j);
    end
...
%plot(t_sol, ST_sol, 'r', 'DisplayName', 'ST');
hold on;
%plot(t_sol, GLU_sol, 'm', 'DisplayName', 'GLU');
hold on;
%plot(t_sol, PYR_sol, 'p', 'DisplayName', 'PYR');
hold on;
...
%Plot the pulse for reference
%figure;
%plot(t, INCW, 'b', 'DisplayName', 'INCW');
%hold on;
%plot(t, INST, 'g', 'DisplayName', 'INST');
%xlabel('Time');
%ylabel('Pulse Signal');
%legend show;
%title('Pulse Signals');
